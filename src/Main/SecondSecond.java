...
public class SecondSecond {
	public  void secondsecond(Nodepair inputNodepair,Cycle cycle,ArrayList<Cycle> cyclelist,ArrayList<Node> inputprotect,ArrayList<Node> inputprotect2)
	{
		ArrayList<Nodepair> Nodepairlist=Main.Nodepairlist;
		String filename="F:\\programFile\\5node\\SSCorss.dat";
...
//						 System.out.println(link1.getName());//TEST
				...
				//*************以上不变
					if(noprotect==0)//若没有冲突则输出节点对 环 与两条路径
					{
							...
							for(int p=0;p<inputprotect2.size()-1;p++)//inputprotect2做第一保护
							{
								filewrite.filewrite(filename,inputprotect2.get(p).getName()+"-");
							}
							filewrite.filewrite(filename,inputprotect2.get(inputprotect2.size()-1).getName()+"    ");
